...
namespace sQzLib
{
    public class QuestionSheetView: StackPanel
    {
        ...
        {
            SingleAnswerMCQView.IdxWidth = idxWidth;
            SingleAnswerMCQView.StemWidth = stemWidth;
            OptionViewFactory.S().StemWidth = stemWidth;
            int idx = -1;
        ...
        void AddListOfSingleQuestions(List<Question> questions, ref int idx_start0, byte[] optionStatusArray = null, bool isEnabled = true)
        {
            foreach (Question q in questions)
                Children.Add(new SingleAnswerMCQView(q, ++idx_start0, optionStatusArray, isEnabled));
        }
    }
}
